...
def simulation2(iter=1, model=model):
    ...
    rand_y_train = pd.Series(model.predict(x_gen_train)) + e_train 
    rand_y_test = pd.Series(model.predict(x_gen_test)) + e_test
...
results_nonlinear = joblib.Parallel(n_jobs = 6, verbose = 13)(joblib.delayed(simulation2)(i) for i in range(n_sim))
...
print('saving files...')
file_name = 'sim_results/sim2_results500_2_0mean.parquet'
